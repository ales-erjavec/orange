...
class TristateCheckItemDelegate(QStyledItemDelegate):
    ...
    def editorEvent(self, event, model, option, index):
        # This is all copied from QStyledItemDelegate.editorEvent.
...
class AddonManagerWidget(QWidget):
    ...
    def __update_details(self):
        ...
        if index == -1:
            ...
            item = toPyObject(item.data(Qt.UserRole))
...
class Installer(QObject):
    ...
    def _next(self):
        def fmt_cmd(cmd):
            return "python " + (" ".join(map(pipes.quote, cmd)))
